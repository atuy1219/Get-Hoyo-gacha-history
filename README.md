...
認証URLは永続保存せず、アプリのプロセス内だけで一時的に保持します。外部サーバーにも送信しません。
...
6. 必要な場合はアプリへ戻り、「認証URLをコピー」を押すとクリップボードへコピーできます。

監視は2分で終了します。履歴画面を既に開いている場合は、一度閉じてから開き直してください。

コピーした認証URLには短時間有効な認証情報が含まれます。第三者へ共有せず、利用後はクリップボードから削除してください。
